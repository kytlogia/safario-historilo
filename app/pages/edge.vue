...
async function checkServerAutoLoadAvailability() {
  ...
  try {
    const body = await $fetch('/api/local-history/edge/status', { query: { profileId } })
...
async function loadEdgeProfiles() {
  try {
    const body = await $fetch('/api/local-history/edge/profiles')
...
async function loadFromServer() {
  ...
  loadError.value = ''
  try {
    const blob = await $fetch<Blob>('/api/local-history/edge', {
...
onMounted(async () => {
  await Promise.all([checkServerAutoLoadAvailability(), loadEdgeProfiles()])
})
