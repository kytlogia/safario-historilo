...
トランクベース開発を採用しています。`develop` や `release/*` のような長命ブランチは作らず、`main` のみを唯一の長命ブランチとします。
...
- 命名規則：`<type>/<issue番号>-<slug>`（`type` は `feature` / `fix` / `chore` / `docs` / `refactor` など）を基本とします。
  ...
  - 共通の必須ルールは「ブランチ名にissue番号を含める」ことのみです。
...
- milestone は Issue 起票時点では未設定でよく、トリアージ時にリポジトリオーナーが設定します。
...
- PRは既定でSquash mergeするため、実質的にはPRタイトルがそのまま `main` 上のコミットメッセージになります。PRタイトルを上記の基準で書けば、個々のコミットメッセージの厳密さは重視しません。
